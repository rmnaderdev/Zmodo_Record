...
RTSP_SERVER = os.environ['RTSP_SERVER']
...
def start_record_process(deviceName, deviceId):
    ...
    command = "ffmpeg -timeout 5000000 -hide_banner -loglevel error -i \"https://flv.meshare.com/live?devid={dev_id}&token={token}&media_type=2&channel=0&rn=1623373509644\" -c copy -an -f rtsp {rtsp_server}/{name}".format(token=TOKEN, name=deviceName, dev_id=deviceId, rtsp_server=RTSP_SERVER)
